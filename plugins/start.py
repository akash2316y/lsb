...
from database.db_fsub import db
   ...
   #      if not is_subscribed:
    ...
    if len(text) > 7:
        try:
            ...
            asyncio.create_task(auto_delete(note_msg, 300))
...
async def is_sub(client: Client, user_id: int, chat_id: int) -> bool:
    """Return True if user_id is currently a member of chat_id."""
    try:
        ...
        print(f"Error checking subscription for {user_id} in {chat_id}: {e}")
        return False


async def check_subscription_status(client: Client, user_id: int, fsub_channels: list):
    """
    Check whether user_id has joined every force-sub channel.
    Returns (is_subscribed, message_text, reply_markup).
    """
    buttons = []
    ...
    for chat_id in fsub_channels:
        ...
        try:
            ...
            if mode == "on" and not chat.username:
                # Request-to-join mode (approved automatically / by admin)
                invite = await client.create_chat_invite_link(
                    chat_id=chat_id,
            ...
            print(f"Error building fsub button for {chat_id}: {e}")

    if not buttons:
        return True, "", None

    buttons.append([InlineKeyboardButton("🔄 ᴛʀʏ ᴀɢᴀɪɴ", callback_data="check_sub")])

    message_text = (
        "<b>⚠️ ᴊᴏɪɴ ᴏᴜʀ ᴄʜᴀɴɴᴇʟ(s) ᴛᴏ ᴜsᴇ ᴛʜɪs ʙᴏᴛ</b>\n\n"
        "<i>Aғᴛᴇʀ ᴊᴏɪɴɪɴɢ, ᴛᴀᴘ Tʀʏ Aɢᴀɪɴ ʙᴇʟᴏᴡ.</i>"
    )
